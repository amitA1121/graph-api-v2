...
export const createEdge = async (node_a_id : NODE_ID_TYPE, node_b_id: NODE_ID_TYPE) => {
    if(node_a_id === node_b_id)
        throw new AppError('edges cant be same',statusCode.BAD_REQUEST, "BAD_REQUEST")

    const a_id = await nodeRepo.getNodeContantById(node_a_id)
    const b_id = await nodeRepo.getNodeContantById(node_b_id)

    if(!a_id) throw new AppError('node A not found',statusCode.NOT_FOUND, "NOT_FOUND")
    if(!b_id) throw new AppError('node B not found',statusCode.NOT_FOUND, "NOT_FOUND")
    
    const isEdgesExisting = await edgeRepo.checkEdgeBetweenTwoNodes(node_a_id, node_b_id)
    if(isEdgesExisting) throw new AppError('edge already existing', statusCode.CONFLICT, "CONFLICT")
...
export const deleteEdge = async(node_a_id: NODE_ID_TYPE, node_b_id: NODE_ID_TYPE) => {
    ...
    if(!isEdgeExist) throw new AppError('edge does not exist',statusCode.NOT_FOUND, "NOT_FOUND")
...
export const getNeighborsOfNode = async (id: NODE_ID_TYPE) => {
    ...
    if(!isNodeExist) throw new AppError('node not found',statusCode.NOT_FOUND, "NOT_FOUND")
